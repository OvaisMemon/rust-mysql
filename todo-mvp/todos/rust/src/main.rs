...
fn add_todo(t: Todo) {
    ...
    for mut stmt in pool.prepare(r"INSERT INTO tbltodo (id, name, done) 
                                    VALUES (:id, :name, :done)")
        .into_iter() {
        for p in obj_todo.iter(){
            stmt.execute(params!{
                "id" => p.id.to_string(),                
                "name" => &p.name,
                "done" => p.done
...
fn remove_todo(id: Uuid) {
    let pool = my::Pool::new("mysql://root:@localhost:3306/todo").unwrap();
    for mut stmt in pool.prepare(r"DELETE FROM tbltodo WHERE id = :id").into_iter(){
        stmt.execute(params!{
            "id" => id.to_string(),
        }).unwrap();
    }
...
fn toggle_todo(id: Uuid) {
    ...
    let todos: Vec<Todo> = pool.prep_exec("SELECT id, name, done from tbltodo WHERE id = :id", params!{ "id" => id.to_string() } )
    .map(|result| {
        result.map(|x| x.unwrap()).map(|row| {                       
            let (id, name, done):(String, String, bool) = my::from_row(row);
            Todo {
    ...
    for mut stmt in pool.prepare(r"UPDATE tbltodo SET done = :done WHERE id = :id").into_iter(){
        stmt.execute(params!{
            "id" => id.to_string(),
            "done" => !todos[0].done
        }).unwrap();
...
fn index() -> ResponseFuture {
    ...
    let pool = my::Pool::new("mysql://root:@localhost:3306/todo").unwrap();
    let todos: Vec<Todo> = pool.prep_exec("SELECT id, name, done from tbltodo", ())    
    .map(|result| {
        result.map(|x| x.unwrap()).map(|row| {                       
            let (id, name, done):(String, String, bool) = my::from_row(row);
            Todo {
    ...
    Box::new(
        req.into_body()
            ...
            .and_then(|whole_body| {
                ...
                add_todo(Todo::new(&words[1].replace("+", " ")));
                redirect_home()
            }),
    )
}
